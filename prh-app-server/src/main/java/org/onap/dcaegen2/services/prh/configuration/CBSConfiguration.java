...
public class CBSConfiguration extends PrhAppConfig {
    ...
    public void runTask() {
        Flux.defer(() -> Mono.just(EnvProperties.fromEnvironment()))
    ...
    private void parsingConfigSuccess(EnvProperties envProperties) {
        ...
        CbsClientFactory.createCbsClient(envProperties)
            .flatMap(cbsClient -> cbsClient.get(RequestDiagnosticContext.create()))
            .subscribe(this::parseCBSConfig, this::cbsConfigError);
    }

    private void parseCBSConfig(JsonObject jsonObject) {
        LOGGER.info("Received application configuration: {}", jsonObject);
        CBSConfigParser cbsConfigParser = new CBSConfigParser(jsonObject);
        dmaapPublisherCBSConfiguration = cbsConfigParser.getDmaapPublisherConfig();
        aaiClientCBSConfiguration = ImmutableAaiClientConfiguration.copyOf(cbsConfigParser.getAaiClientConfig())
            .withAaiHeaders(aaiClientConfiguration.aaiHeaders());
        dmaapConsumerCBSConfiguration = cbsConfigParser.getDmaapConsumerConfig();
    }
