...
public class AAIConsumerTaskImpl extends AAIConsumerTask<AAIClientConfiguration> {
    ...
    protected void consume() throws AAINotFoundException {
        logger.debug("Start task AAIConsumerTask::publish() :: {}", dateTimeFormatter.format(
            LocalDateTime.now()));
    ...
    public Object execute(Object object) throws AAINotFoundException {
        logger.debug("Start task AAIProducerTaskImpl::execute() :: {}", dateTimeFormatter.format(
            LocalDateTime.now()));
        consume();
        logger.debug("End task AAIPublisherTaskImpl::execute() :: {}", dateTimeFormatter.format(
            LocalDateTime.now()));
        return null;
    }
    ...
    @Override
    void initConfigs() {
        logger.debug("initConfigs for AAIConsumerTaskImpl not needed/supported");
    }
