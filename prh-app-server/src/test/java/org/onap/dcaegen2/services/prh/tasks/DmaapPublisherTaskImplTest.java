...
class DmaapPublisherTaskImplTest {
    ...
    public void beforeEach() throws SSLException {
        ...
        when(publisherReactiveHttpClientFactory.create(dmaapPublisherConfiguration)).thenReturn(dMaaPPublisherReactiveHttpClient);
    }
    ...
    @Test()
    void execute_whenConsumerDmaapModelIsNull() throws DmaapNotFoundException, SSLException {
        //given
        HttpResponseStatus httpResponseStatus = HttpResponseStatus.UNAUTHORIZED;
        HttpClientResponse httpClientReponse = prepareMocksForTests(httpResponseStatus);
        dmaapPublisherTask = new DmaapPublisherTaskImpl(appConfig,publisherReactiveHttpClientFactory);
        ...
        //when
        StepVerifier.create(dmaapPublisherTask.execute(null)).expectSubscription()
            .expectNext(httpClientReponse);
    ...
    private HttpClientResponse prepareMocksForTests(HttpResponseStatus httpResponseStatus) {
        ...
        when(dMaaPPublisherReactiveHttpClient.getDMaaPProducerResponse(dmaapModel, requestDiagnosticContextOptionalMock))
